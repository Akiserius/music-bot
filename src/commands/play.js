...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName("name").setDescription("Bir şarkı adı girin!").setRequired(true)),
...
if (!tracks) return interaction.followUp("🎵 | Müzik Oynatılıyor...")
const embed = new Discord.EmbedBuilder()
.addFields({name: "Şarkı", value: `${tracks.title}`, inline: true})
.addFields({name: "Şarkıcı", value: `${tracks.author}`, inline: true})
.addFields({name: "Süre", value: `${tracks.duration}`, inline: true})
.addFields({name: "Görüntülenme", value: `${tracks.views}`, inline: true})
.addFields({name: "Thumbnail", value: "[Tıkla]("+tracks.thumbnail+")", inline: true})
...
.addComponents(
  ...
  .setURL("https://discord.gg/q5dErq8bdj")
)
